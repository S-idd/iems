...
The original command below is the verified macOS ARM64 rehearsal. The trust-bound [Linux integrated acceptance](linux-integrated-acceptance.md) also passed all 13 IEMS scenarios on AlmaLinux x86-64 under WSL2 on 2026-09-22.
...
This proves local development behavior on macOS ARM64 and AlmaLinux x86-64 under WSL2. AI is advisory only; score calibration is unverified. Runtime evidence proves publisher handoff, not Kafka delivery. Webhook retry was previously demonstrated as **manual** retry, not automatic retry. The physical migration gate covers the governed SQLite runner, not unrestricted direct SQL. PostgreSQL/MySQL and bare-metal Linux acceptance remain incomplete. The model supplies risk labels and scores, not explanations or corrections. The package is an unpublished development build; no tag, release or deployment is part of this rehearsal.
